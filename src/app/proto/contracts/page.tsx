...
import { getContracts } from '@/mock/contracts'
...
const REQUIRED_TYPES = [
  ...
  { key: 'LEJEKONTRAKT',         label: 'Lejekontrakt' },
  { key: 'FORSIKRING',           label: 'Forsikring' },
  { key: 'VEDTAEGTER',           label: 'Vedtægter' },
  { key: 'ANSAETTELSESKONTRAKT', label: 'Ansættelse' },
  { key: 'DRIFTSAFTALE',         label: 'Drift' },
] as const
...
// ---------------------------------------------------------------
// Udledet status (3 niveauer) — encoder urgency
// ---------------------------------------------------------------
...
function sortContracts(items: MockContract[], key: SortKey, dir: 'asc' | 'desc'): MockContract[] {
...
export default function ContractsPage() {
  const { dataScenario, companyCount } = usePrototype()

  const allContracts = useMemo(() => getContracts(dataScenario), [dataScenario])
  const companies = useMemo(() => getCompanies(dataScenario, companyCount), [dataScenario, companyCount])
  ...
  const missingRows = useMemo(() => {
    ...
    for (const company of companies) {
      const companyTypes = new Set(allContracts.filter((c) => c.companyId === company.id).map((c) => c.systemType))
  ...
  const counts = useMemo(() => {
    return {
      expired:  allContracts.filter((c) => deriveStatus(c) === 'expired').length,
      expiring: allContracts.filter((c) => deriveStatus(c) === 'expiring').length,
      active:   allContracts.filter((c) => deriveStatus(c) === 'active').length,
      missing:  missingRows.length,
    }
  }, [allContracts, missingRows.length])

  // Urgency items til pinned panel (max 5)
  const urgencyItems = useMemo(() => {
    const expired = allContracts
    ...
    const items: Array<
      | { kind: 'contract'; contract: MockContract; status: DerivedStatus }
  ...
  return (
    <div className="min-h-full bg-slate-50/60 p-8">
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[13px] text-slate-500 mt-1">
              {allContracts.length} kontrakter i porteføljen
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-slate-400 truncate">
                          {item.contract.companyName} · {item.contract.categoryLabel}
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-xl ring-1 ring-slate-900/[0.06] shadow-[0_1px_2px_rgba(15,23,42,0.04)] [overflow:clip]">
                  ...
                  return (
                    <Fragment key={c.id}>
                      ...
                      <tr
                        ...
                        // Native browser virtualization — skalerer til 1000+ rækker
                        style={{
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-slate-600">
                          <Link href={`/proto/portfolio/${c.companyId}`} className="text-slate-600 hover:text-slate-900 no-underline">
                            {c.companyName.replace(' ApS', '')}
                          </Link>
                        </td>
                        <td className="px-4 py-3">
            ...
            {sorted.length > 0 && (
              <div className="px-5 py-3.5 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                <span>
                  {sorted.length === allContracts.length
                    ? `${sorted.length} kontrakter · Slut på listen`
                    : `Viser ${sorted.length} af ${allContracts.length} kontrakter`}
                </span>
        ...
        {/* MATRIX VIEW */}
        {view === 'matrix' && <MatrixView companies={companies} contracts={allContracts} />}
      </div>
...
function cellFor(companyId: string, typeKey: string, contracts: MockContract[]): { status: CellStatus; contract?: MockContract } {
  const matches = contracts.filter((c) => c.companyId === companyId && c.systemType === typeKey)
  if (matches.length === 0) return { status: 'missing' }
  const sorted = [...matches].sort((a, b) => {
    const rank = (c: MockContract) => {
            ...
            {shownCompanies.map((co) => (
              <tr key={co.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/40">
                <td className="sticky left-0 bg-white/95 backdrop-blur-sm px-4 py-2.5 whitespace-nowrap">
                  <Link href={`/proto/portfolio/${co.id}`} className="text-slate-900 font-medium no-underline hover:text-slate-950">
                    {co.name.replace(' ApS', '')}
                  </Link>
                  <div className="text-[10px] text-slate-400 mt-0.5">{co.city}</div>
                </td>
                {REQUIRED_TYPES.map((t) => {
                        ...
                        <div className="absolute z-20 top-[calc(100%+6px)] left-1/2 -translate-x-1/2 bg-white rounded-lg p-3 min-w-[220px] ring-1 ring-slate-900/10 shadow-[0_12px_32px_-12px_rgba(15,23,42,0.3)] text-left">
                          <div className="flex items-center justify-between mb-2">
                            <div className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.08em]">
                              {co.name.replace(' ApS', '')} · {t.label}
                            </div>
                            <button onClick={() => setPopover(null)} className="text-slate-400 hover:text-slate-900">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                          {cell.contract ? (
                            <>
                              ...
                              <Link
                                href={`/proto/contracts/${cell.contract.id}`}
                            ...
                            <>
                              ...
                              <button className="mt-3 px-2.5 py-1.5 rounded-md bg-slate-900 text-white text-[11px] font-medium hover:bg-slate-800">

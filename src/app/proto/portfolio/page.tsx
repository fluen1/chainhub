...
import { getPortfolioTotals, getFinancialByCompany } from '@/mock/financial'
...
// Kort "region" → by-clustering (for byer med mange lokationer)
// Ikke relevant endnu (22 byer = 22 lokationer), men struktureret til udvidelse

// ---------------------------------------------------------------
// Grupper selskaber efter by
// ---------------------------------------------------------------
interface CityCluster {
  city: string
  companies: MockCompany[]
  worstStatus: MockCompany['healthStatus']
  position: { top: string; left: string }
}

function groupByCities(companies: MockCompany[]): CityCluster[] {
  const map = new Map<string, MockCompany[]>()
  ...
  return Array.from(map.entries())
    .map(([city, list]) => {
      const worstStatus = list.reduce<MockCompany['healthStatus']>((worst, c) => {
...
// Farver
// ---------------------------------------------------------------
function dotColor(status: MockCompany['healthStatus']): string {
  switch (status) {
    case 'critical':
...
function badgeColor(status: MockCompany['healthStatus']): string {
  switch (status) {
    case 'critical':
...
function badgeLabel(status: MockCompany['healthStatus']): string {
  switch (status) {
    case 'critical':
      return 'Kritisk'
    case 'warning':
...
function CityClusterDot({ cluster }: { cluster: CityCluster }) {
  ...
  return (
    <div
      ...
      style={{ top: cluster.position.top, left: cluster.position.left, transform: 'translate(-50%, -50%)' }}
    >
      ...
      <div className="absolute bottom-[calc(100%+10px)] left-1/2 -translate-x-1/2 bg-white rounded-lg p-2.5 min-w-[240px] ring-1 ring-slate-900/5 shadow-[0_12px_32px_-12px_rgba(15,23,42,0.25)] opacity-0 translate-y-1 pointer-events-none group-hover/cluster:opacity-100 group-hover/cluster:translate-y-0 group-hover/cluster:pointer-events-auto transition-all duration-150 z-20">
        <div className="text-[9px] font-medium text-slate-400 uppercase tracking-[0.08em] mb-1.5 px-1">
          {cluster.city} · {cluster.companies.length} {cluster.companies.length === 1 ? 'lokation' : 'lokationer'}
        </div>
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[10px] text-slate-500 truncate">
                {c.partnerName} · {c.partnerOwnershipPct}%
              </div>
            </div>
            <span className={cn('text-[9px] font-medium px-1.5 py-0.5 rounded shrink-0', badgeColor(c.healthStatus))}>
...
function PanelCard({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('bg-white rounded-xl p-4 ring-1 ring-slate-900/[0.06] shadow-[0_1px_2px_rgba(15,23,42,0.04)]', className)}>
      <div className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.1em] mb-3">{title}</div>
      {children}
    </div>
  )
}

function KpiBox({ label, value, trend, trendDir, danger }: {
  label: string
  value: string
  unit?: string
  trend?: string
  ...
  return (
    <div className="rounded-lg p-3 bg-slate-50/70">
      ...
      <div className={cn('text-[20px] font-semibold leading-tight tabular-nums mt-1', danger ? 'text-rose-600' : 'text-slate-900')}>
        {value}
      </div>
      {trend && (
        <div
...
// ---------------------------------------------------------------
type SortKey = 'name' | 'city' | 'partner' | 'ownership' | 'revenue' | 'margin' | 'status' | 'contracts' | 'cases'

function getRevenue(companyId: string): number {
  const fin = getFinancialByCompany(companyId).find((f) => f.year === 2025)
  return fin?.omsaetning ?? 0
}

function getMargin(companyId: string): number {
  const fin = getFinancialByCompany(companyId).find((f) => f.year === 2025)
  if (!fin || !fin.omsaetning || fin.ebitda == null) return 0
  return fin.ebitda / fin.omsaetning
}

function sortCompanies(companies: MockCompany[], key: SortKey, dir: 'asc' | 'desc'): MockCompany[] {
  ...
  return [...companies].sort((a, b) => {
    ...
    switch (key) {
      case 'name':
        cmp = a.name.localeCompare(b.name, 'da'); break
      case 'city':
        cmp = a.city.localeCompare(b.city, 'da'); break
      case 'partner':
        cmp = a.partnerName.localeCompare(b.partnerName, 'da'); break
      case 'ownership':
        cmp = a.groupOwnershipPct - b.groupOwnershipPct; break
      case 'revenue':
        cmp = getRevenue(a.id) - getRevenue(b.id); break
      case 'margin':
        cmp = getMargin(a.id) - getMargin(b.id); break
      case 'status':
        cmp = statusRank[a.healthStatus] - statusRank[b.healthStatus]; break
      case 'contracts':
        cmp = a.contractCount - b.contractCount; break
      case 'cases':
        cmp = a.openCaseCount - b.openCaseCount; break
    }
    return cmp * mult
  })
}
...
export default function PortfolioPage() {
  ...
  const allCompanies = useMemo(
    () => filterCompaniesByRole(getCompanies(dataScenario, companyCount), role, activeUser.companyIds),
    [dataScenario, companyCount, role, activeUser.companyIds],
  )
  ...
  }, [allCompanies, search, healthFilter])
  ...
  const counts = useMemo(() => {
    return {
      critical: allCompanies.filter((c) => c.healthStatus === 'critical').length,
      warning: allCompanies.filter((c) => c.healthStatus === 'warning').length,
      healthy: allCompanies.filter((c) => c.healthStatus === 'healthy').length,
    }
  }, [allCompanies])

  const totals = getPortfolioTotals(2025)
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[13px] text-slate-500 mt-1">
              {allCompanies.length} lokationer i porteføljen
              {totalAttention > 0 && <> · <span className="text-slate-700 font-medium">{totalAttention} kræver handling</span></>}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid grid-cols-[1fr_340px] gap-4">
          ...
          >
            ...
            <div
              ...
              style={{
                background: 'radial-gradient(ellipse at 50% 40%, rgba(148,163,184,0.04) 0%, transparent 60%)',
              }}
            />
            {/* Grid pattern — meget subtil */}
            <div
          ...
          <div className="flex flex-col gap-3">
            ...
            <PanelCard title="Portefølje">
              <div className="grid grid-cols-2 gap-2.5">
                <KpiBox label="Lokationer" value={String(allCompanies.length)} trend="+2 i år" trendDir="up" />
                <KpiBox label="Kræver handling" value={String(totalAttention)} trend="↑1 fra sidste uge" trendDir="down" danger />
                <KpiBox label="Omsætning" value={`${formatMio(totals.totalOmsaetning)}M`} trend="+12% YoY" trendDir="up" />
                <KpiBox
                  label="EBITDA margin"
                  value={`${(totals.avgEbitdaMargin * 100).toFixed(1)}%`}
                  trend="-0.8pp"
                  trendDir="down"
                />
              </div>
              ...
              <div className="space-y-1.5">
                {urgencyItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-slate-400 font-medium whitespace-nowrap tabular-nums">{item.time}</div>
                  </Link>
                ))}
            ...
            <PanelCard title="Seneste aktivitet">
              <div className="space-y-1">
                <ActivityItem
                  ...
                  meta="Odense Tandlægehus · AI-behandlet · 2t siden"
                />
                <ActivityItem
                  color="#3b82f6"
                  text="Driftsbesøg gennemført"
                  meta="Aarhus Tandklinik · i går"
                />
                <ActivityItem
                  color="#22c55e"
                  text="Forsikring fornyet automatisk"
                  meta="Aalborg Tandlægehus · 2d siden"
                />
              </div>
            </PanelCard>
          </div>
        ...
        <table className="w-full text-[12px]">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/50">
              <Th label="Selskab"    active={sortKey === 'name'}      dir={sortDir} onClick={() => onSort('name')} />
              <Th label="By"         active={sortKey === 'city'}      dir={sortDir} onClick={() => onSort('city')} />
              <Th label="Partner"    active={sortKey === 'partner'}   dir={sortDir} onClick={() => onSort('partner')} />
              <Th label="Ejerskab"   active={sortKey === 'ownership'} dir={sortDir} onClick={() => onSort('ownership')} align="right" />
              <Th label="Omsætning"  active={sortKey === 'revenue'}   dir={sortDir} onClick={() => onSort('revenue')}   align="right" />
              <Th label="EBITDA %"   active={sortKey === 'margin'}    dir={sortDir} onClick={() => onSort('margin')}    align="right" />
              <Th label="Kontrakter" active={sortKey === 'contracts'} dir={sortDir} onClick={() => onSort('contracts')} align="right" />
              <Th label="Sager"      active={sortKey === 'cases'}     dir={sortDir} onClick={() => onSort('cases')}     align="right" />
              <Th label="Status"     active={sortKey === 'status'}    dir={sortDir} onClick={() => onSort('status')} />
            </tr>
          </thead>
          <tbody>
            {companies.map((c) => {
              const revenue = getRevenue(c.id)
              const margin = getMargin(c.id)
              return (
                <tr
                  key={c.id}
                ...
                >
                  <td className="px-4 py-3">
                    <Link href={`/proto/portfolio/${c.id}`} className="font-medium text-slate-900 group-hover/row:text-slate-950 no-underline">
                      {c.name}
                    </Link>
                    <div className="text-[10px] text-slate-400 mt-0.5 tabular-nums">CVR {c.cvr}</div>
                  </td>
                  <td className="px-4 py-3 text-slate-600">{c.city}</td>
                  <td className="px-4 py-3 text-slate-600">{c.partnerName}</td>
                  <td className="px-4 py-3 text-slate-600 text-right tabular-nums">{c.groupOwnershipPct}%</td>
                  <td className="px-4 py-3 text-slate-900 font-medium text-right tabular-nums">
                    {revenue > 0 ? `${formatMio(revenue)}M` : '—'}
                  </td>
                  <td className="px-4 py-3 text-slate-600 text-right tabular-nums">
                    {revenue > 0 ? `${(margin * 100).toFixed(1)}%` : '—'}
                  </td>
                  <td className="px-4 py-3 text-slate-600 text-right tabular-nums">{c.contractCount}</td>
                  <td className="px-4 py-3 text-right tabular-nums">
                    <span className={cn(c.openCaseCount > 0 ? 'text-rose-600 font-medium' : 'text-slate-400')}>
                      {c.openCaseCount}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn('inline-flex items-center gap-1.5 text-[10px] font-medium px-2 py-0.5 rounded', badgeColor(c.healthStatus))}>
                      <span
                        className={cn(
...
// Activity item
// ---------------------------------------------------------------
function ActivityItem({ color, text, meta }: { color: string; text: string; meta: string }) {
  return (
    <div className="flex gap-2.5 py-2">
      <div className="w-1.5 h-1.5 rounded-full mt-[7px] shrink-0" style={{ backgroundColor: color }} />
      <div className="min-w-0">
        <div className="text-[12px] font-medium text-slate-800 leading-snug">{text}</div>

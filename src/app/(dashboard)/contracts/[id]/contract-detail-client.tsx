...
export interface ContractDetailData {
  ...
  status: string
  statusLabel: string
  derivedStatus: 'expired' | 'expiring' | 'active'
  systemType: string
...
export interface ActivityItem {
  dotColor: 'violet' | 'blue' | 'red' | 'green' | 'amber'
  text: string
  meta: string
}
...
type DerivedStatus = ContractDetailData['derivedStatus']
    ...
    >
      <header className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          {badge && (
            <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded', badgeClass)}>
              {badge.label}
            </span>
          )}
        </div>
...
function KeyTerm({
  ...
  return (
    <div>
      <div className="text-[10px] font-medium text-violet-700 uppercase tracking-[0.08em] mb-0.5">
        {label}
      </div>
      <div
        className={cn(
...
export default function ContractDetailClient({
  ...
  relatedContracts: sameCategory,
  activity: activityFeed,
  totalDocuments,
}: ContractDetailClientProps) {
  const derived = contract.derivedStatus
  ...
  const criticalInsight = (() => {
    if (derived === 'expired') {
      return {
        ...
        body: `Denne ${contract.categoryLabel.toLowerCase()} udløb for ${contract.daysUntilExpiry != null ? Math.abs(contract.daysUntilExpiry) : '?'} dage siden. Selskabet er potentielt ikke dækket — start genforhandling umiddelbart eller opret opfølgningsopgave.`,
      }
    }
    if (derived === 'expiring') {
      return {
  ...
  // Primary CTA
  const primaryCta = { label: 'Opret opgave', icon: Plus }
  if (derived === 'expired') {
    primaryCta.label = 'Opret opfølgningsopgave'
  }
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-4 text-[12px] text-slate-500 mt-1.5 flex-wrap">
                <Link
                  ...
                  className="text-slate-500 hover:text-slate-900 no-underline"
                >
                  {contract.companyName}
                </Link>
                <span>{contract.categoryLabel}</span>
                <span>Følsomhed: {contract.sensitivityLabel}</span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 ring-1 ring-violet-200/60 rounded-xl p-5 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="text-[10px] font-medium text-violet-700 uppercase tracking-[0.08em]">
              Nøglevilkår{keyTerms.hasExtractionData ? ' · AI-ekstraheret' : ' · Kontraktdata'}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            ...
            <KeyTerm
              label="Udløb"
              value={keyTerms.udloeb}
              danger={derived === 'expired'}
              warning={derived === 'expiring'}
            />
        ...
        <div className="grid grid-cols-[180px_1fr] gap-8">
          ...
          <div className="min-w-0 flex flex-col gap-4">
            ...
            >
              {relatedCases.length === 0 && (
                <p className="text-[12px] text-slate-400 py-1">
                  Ingen relaterede sager på denne kontrakt.
                </p>
              )}
            ...
            <Section
              ...
              action={
                totalDocuments > 0
                  ? { label: `Alle ${totalDocuments}`, href: '/documents' }
                  : undefined
              }
            >
